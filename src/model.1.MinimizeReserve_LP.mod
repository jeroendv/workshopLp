...
# number of primary choices. Assigning people to a secondary choice must be
# avoided if possible.
#
#
## Modelling
...
Overbookings{w in W, s in S}:
     Capacity[w,s] >=  sum{p in P} assign[w,s,p];
...
param assigned{w in W, s in S} := sum{p in P} assign[w,s,p];
...
printf: '\n\n';
printf: 'unassigned primary choices:\n';
printf: '--------------------------\n' ;
